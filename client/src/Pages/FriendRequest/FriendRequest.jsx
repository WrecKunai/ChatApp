...
const FriendRequest = ({req,change,accept,pending}) => {
  const {user}=useContext(AuthContext)
    ...
    const removeFriendRequest=async(cr)=>{
      try{
        const res=await axios.put(`${URL}/api/Auth/removerequest/${user._id}/${cr._id}`)
        change()
        accept(cr._id)
        pending(cr._id)
      }
    ...
    const removePending=async(cr)=>{
     try{
        const res=await axios.put(`${URL}/api/Auth/removepending/${cr._id}/${user._id}`)
      }
      catch(err){
        console.log(err)
      }
    ...
    const checkconversation=async(cr)=>{
      try{
        const res=await axios.get(`${URL}/api/Conversation/${cr._id}/${user._id}`)
  ...
  return (
    ...
    {data[0]&&data.map((cr)=>(<div className="fr-wrapper">
    <div className="userdet">
    <img className='img-fr' src={cr.profileImg} alt="..." onError={event => {event.target.src =`${URL}/public/images/7.jpeg`
     event.onerror = null}}/>
    <p>{cr.Firstname}</p>
    </div>
