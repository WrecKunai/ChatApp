...
useEffect(()=>{
  allusers?setalusers(allusers.map((cr)=>({label:cr.username}))):console.log("koko")
},[allusers])
  ...
  useEffect(()=>{
    arrivalmsg?setmessages((prev)=>[...prev,arrivalmsg]):console.log("null")
  },[arrivalmsg,trigger])
useEffect(()=>{
  const fetchuser=()=>{
  
    // console.log()
     setuserdetail(allusers.filter((cr)=>
     search.label===cr.username
    ))
   
}
allusers?fetchuser():console.log("meow")
},[search])
...
const confirm=async ()=>{
  receiverId=conversation[0].members.find((member)=>member!==user._id)
...
const onl=(filter)=>{
  const array=filter.filter((cr)=>friends.some((curr)=>cr.userId===curr))
  console.log(array)
...
const check=()=>{
  ...
  return (
   ...
   <div className="main">
<div onClick={check} className="button">Testing</div>
  <div className={toggletop?"navbar-hide nav-top":"navbar-show nav-top"}>
  <div className={active?"btn-toggle btn-top":"btn-hide btn-top"}  onClick={()=>{tog(); toggletopbar()}}><ArrowBackIcon/></div>
  <Navbar toggle={tog} button={active} />
  </div>
    
    <div className={toggletop?"visible navtoggle":"hide navtoggle"}>
    <div className="btn-toggle btn-top2"  onClick={()=>{tog(); toggletopbar()}}><ArrowBackIcon/></div>
...
>
      ...
      <Collapse in={open} timeout="auto" unmountOnExit>
        <List component="div" disablePadding>
          <ListItemButton sx={addfr?{bgcolor: "#5c5e5d" , pl: 4 }:{bgcolor: 'background.paper', pl: 4  }}  onClick={handleadd} >
            <ListItemIcon>
              <PersonAddIcon />
            </ListItemIcon>
            <ListItemText primary="Add Friend" />
          </ListItemButton>
          <ListItemButton sx={request?{bgcolor: "#5c5e5d",pl: 4  }:{bgcolor: 'background.paper',pl: 4  }}  onClick={handlereq} >
        <ListItemIcon>
          <ForwardToInboxIcon />
        </ListItemIcon>
        <ListItemText primary="Friend Request" />
      </ListItemButton>
      <ListItemButton sx={pending?{bgcolor: "#5c5e5d",pl: 4  }:{bgcolor: 'background.paper',pl: 4  }}  onClick={handlepending} >
        <ListItemIcon>
          <PendingIcon />
        </ListItemIcon>
   ...
   {friends.map((c,i)=>(
      ...
      <Friends  key={i} change={triggerr} changeconvo={handle} remove={sendfriendremoveid} online={online} refresh={refreshmsg} toggle1={tog} current={c}/>
      </div>
    ))}
    ...
    {active?request?
    friendrequest[0]?<div>
        {friendrequest.map((cr,i)=>(<FriendRequest key={i} pending={sendpendingid} accept={sendfriendacceptid} change={triggerr} req={cr}/>))}
      </div>:<div className='alt-text'>"No friend request"</div>:pending?
      sentrequest?<div><div className="header">
      <p className='hd-title'>Pending</p>
      <hr />
    </div>{sentrequest.map((cr,i)=>(<SentRequest key={i} sent={cr}/>))}</div>:<div className='alt-text'>"No Pending requests"</div>:<div className='Add friend'><AddFriend  change={triggerr} request={sendfriendreqid} user={User[0]}/></div>:
     conversationactive?<div className="msg-main">
     {messages[0]?<div  className="cont-msg">
    {messages.map((cr,i)=>(
    <div key={cr._id} ref={element} className="messages">
    <Messages prop={cr} chk={cr.senderid===user._id}/>
   </div>
    ))}
    </div>:<div className='alt-text'>"no messages here"</div>}
    <div className="input">
    <input className="input-field" id="newmessage" onChange={handleChange} value={newmsg.newmessage} placeholder="Type your message" type="text" />
    <div onClick={confirm} className="btn-send"><SendIcon/></div>
    </div>
    </div>:<div className='alt-text'><h1>CLICK TO START A CONVERSATION</h1></div>}
    </div>
   <div className="online-cont">
      <p>Online</p>
      <hr />
   {onlinefilter&&onlinefilter.map((cr)=>(   
  <div  className="me">
